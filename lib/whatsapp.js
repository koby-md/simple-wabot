 ...
 * @itsliaaa/starcore
 *
 * Buttons use Native Flow by default.
 */
...
export function sameJid(conn, a, b) {
  ...
  /*
   * LID must remain exact.
   */
  if (isLid(A) || isLid(B))
    return A === B

  /*
   * Group/newsletter/broadcast exact.
   */
  if (
    isGroupJid(A) ||
    isGroupJid(B) ||
 ...
 * IMPORTANT:
 *
 * We use nativeFlow here instead of the old
 * buttons property.
 *
 * @itsliaaa/baileys supports nativeFlow:
 *
 * nativeFlow: [
 *   {
 *     text: 'Yes',
