...
export function makeWASocket(connectionOptions, options = {}) {
 ...
 sendFile: {
async value(jid, path, filename = '', caption = '', quoted = null, ptt = false, options = {}) {

  let type = await conn.getFile(path, true)
  let { res, data: file, filename: pathFile } = type

  if (!file || !file.length) throw '❌ Archivo vacío'
  ...
  let mtype = 'document'
  let mimetype = options.mimetype || type.mime || 'application/octet-stream'

  if (options.asDocument) {
    mtype = 'document'
  } else if (options.asSticker || /webp/i.test(mimetype)) {
    mtype = 'sticker'
  } else if (options.asImage || /image/i.test(mimetype)) {
    mtype = 'image'
  } else if (options.asVideo || /video/i.test(mimetype)) {
    mtype = 'video'
  } else if (options.asAudio || /audio/i.test(mimetype)) {
    mtype = 'audio'
  }
...
if (mtype === 'audio') {
  ...
  return await conn.sendMessage(jid, message, { quoted: q })
},
enumerable: true
},
    ...
    enumerable: true
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options) {
                // كنفحصو واش الشات عبارة عن كروب
                const isGroup = jid && jid.endsWith('@g.us');
                // يلا كان الشات خاص (ماشي كروب)، كنلغيو الرد المقتبس
                const finalQuoted = isGroup ? quoted : null;

                return Buffer.isBuffer(text) 
                    ? conn.sendFile(jid, text, 'file', '', finalQuoted, false, options) 
                    : conn.sendMessage(jid, { ...options, text }, { quoted: finalQuoted, ...options });
            }
        },

sendButton: {
  async value(jid, text = '', footer = '', quoted, buttons = [], options = {}) {
    ...
    const msg = generateWAMessageFromContent(jid, message, { quoted })
...
getJid: {
    async value(sender) {
        ...
        // Si NO es LID devolver normal
        if (!jid.endsWith('@lid')) return jid

        // Cache inicializar si no existe
...
getNum: {
  async value(jid, chatId = null) {
    ...
    if (jid.endsWith('@lid')) {
      ...
      // 1️⃣ Cache
      if (this.lidCache.has(jid)) {
        realJid = this.lidCache.get(jid)
      }

      // 2️⃣ signalRepository (más rápido y limpio)
      else {
        try {
          const pn = await this.signalRepository?.lidMapping?.getPNForLID(jid)
          if (pn?.endsWith('@s.whatsapp.net')) {
            this.lidCache.set(jid, pn)
            realJid = pn
          }
        } catch {}

        // 3️⃣ Fallback metadata si aún no resolvió
        if (realJid === jid && chatId) {
          try {
            const metadata = await this.groupMetadata(chatId)

            for (const p of metadata.participants) {
              if (p.id === jid && p.phoneNumber) {
                this.lidCache.set(jid, p.phoneNumber)
                realJid = p.phoneNumber
                break
              }
            }
          } catch {}
  ...
  async value(jid = '', withoutContact = false) {
    try {
      ...
      if (jid.endsWith('@g.us')) {
        let group = this.chats?.[jid]
        if (group?.subject) return group.subject

        let meta = await this.groupMetadata(jid).catch(() => null)
        return meta?.subject || 'Grupo'
      }

      // Bot
      if (jid === this.user?.jid || jid === this.user?.id) {
        return this.user?.name || 'Bot'
      }

      let chat = this.chats?.[jid] || {}
      let contact = this.contacts?.[jid] || {}
...
//----
  loadMessage: {
     ...
     */
    async value(jid, messageID) {
        try {
            if (!jid || !messageID) return null

            const chat = conn.messages?.[jid]
            if (!chat) return null
            ...
            console.error('loadMessage error:', e)
            return null
        }
    },
    enumerable: true
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
    get() {
        let jid = this.key?.fromMe
            ? this.conn?.user?.id
            : this.participant || this.key?.participant || this.chat || ''

        if (!jid) return jid
        return jid.decodeJid() // تم التعديل هنا لتفادي حذف الدومين
    },
    enumerable: true
},
       ...
       name: {
    get() {
        return this.pushName || this.sender?.split('@')[0] || 'Usuario'
    }, 
    enumerable: true
},
...
download: {
    ...
    async value(text, chatId, options = {}) {
        ...
        // يلا كان الشات خاص، البوت غيصيفط الميساج ديريكت بلا quoted
        const finalQuoted = isGroup ? this : null;
